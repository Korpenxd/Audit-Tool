...
test("both themes and responsive header controls are styled", async () => {
  ...
  assert.match(css, /\.site-header-inner \{[^}]*flex-direction: row[^}]*flex-wrap: nowrap/);
  assert.match(css, /data-theme="light"\] \.architecture/);
  ...
  assert.match(page, /className="architecture"/);
